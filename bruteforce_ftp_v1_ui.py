...
def brute_force_ftp(host, username, password_file, max_workers=5, delay=1):
...
def start_brute_force():
    ...
    port = 21 if ftp_var.get() else 22
...
def check_server_and_start_attack(host, port, username, password_file):
    ...
    if not check_server_availability(host, port):
        log_message(f"Le serveur FTP n'est pas disponible ou le port {port} n'est pas ouvert.", "red")
        stop_brute_force()  # Arrêter si le serveur n'est pas disponible
        return

    log_message("Démarrage de l'attaque de bruteforce sur FTP...", "blue")
...
class FTPBruteforceApp:
    def __init__(self, root):
        ...
        # Ajouter l'icône
        icon_path = "F:/Documents/07_projet_python/programmes_ok/logos/bruteforce_ftp.ico"
