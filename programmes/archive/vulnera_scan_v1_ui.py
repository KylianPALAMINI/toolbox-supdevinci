...
def verifier_cle_api(text_widget):
    try:
        # vérifier accès à l'API Shodan
        info = api.info()
        text_widget.insert(tk.END, "Accès à l'API Shodan vérifié avec succès.\n", "success")
        text_widget.insert(tk.END, "Infos sur l'API Shodan:\n", "info")
        text_widget.insert(tk.END, f"{info}\n", "info")
        if info['query_credits'] > 0:
            return True
        else:
            text_widget.insert(tk.END, "Pas assez de crédits de requête Shodan.\n", "error")
            return False
...
def recherche_shodan(cible, text_widget):
    try:
        # recherche d'infos sur la cible avec Shodan
        host = api.host(cible)
        results = []
        ...
        # afficher ports ouverts et services
...
class VulnScannerApp:
    def __init__(self, root):
        ...
        icon_path = os.path.join(base_dir, "logos", "scan_vuln.ico")
